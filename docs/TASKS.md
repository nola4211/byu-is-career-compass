...
| LK-003 | High | Integrate the live voice session UI | Codex | In progress | Real microphone test | Student can start/end, mute, see state/errors, open the transcript on demand, and use written fallback | PR #9 and Pages workflow `34398273576` deployed the verified water UI; microphone conversation remains pending |
...
| UX-001 | High | Simplify the live interview presentation | Codex | Done | None | Active voice practice emphasizes one water presence; listening is calm, agent speech is audio-reactive, and transcript is opt-in | PR #9 merged as `f7ac294`; workflow `34398273576` and deployed-asset check passed |
