...
| LK-003 | High | Integrate the live voice session UI | Codex | In progress | LK-002 for end-to-end test | Student can start/end, mute, see state/transcript/errors, and use written fallback | First production test exposed an immediate cleanup disconnect; focused fix passes lint and TypeScript, pending deploy and retest |
| LK-004 | High | Pass career-aware session metadata to the agent | Codex | Blocked | Active LiveKit deployment | All eight IDs and both modes are validated; derived metadata reaches agent | Matching Agent Builder config exists, but dashboard shows no agent name/version and `Not deployed yet`; deploy it, then verify metadata in LiveKit logs |
